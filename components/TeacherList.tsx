...
const TeacherList: React.FC<TeacherListProps> = ({ userRole, onNavigate }) => {
  ...
  const handleBookClick = (teacher: Teacher, e?: React.MouseEvent) => {
    e?.stopPropagation();
    if (!userRole) {
      alert("Please sign up or login to book a session.");
  ...
  const handleConfirmBooking = (e: React.FormEvent) => {
    e.preventDefault();
    alert(`Booking confirmed with ${selectedTeacher?.name}! See you in the dashboard.`);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div className="space-y-6">
                            <div>
                                ...
                                <input type="text" required value={editorData.name || ''} onChange={e => setEditorData({...editorData, name: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Category</label>
                                    <select value={editorData.category} onChange={e => setEditorData({...editorData, category: e.target.value as any})} className="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-900">
                                        {CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Price</label>
                                    <input type="text" value={editorData.price || ''} onChange={e => setEditorData({...editorData, price: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-900" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Bio Summary</label>
                                <input type="text" value={editorData.specialty || ''} onChange={e => setEditorData({...editorData, specialty: e.target.value})} className="w-full px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-900" />
                            </div>
                        </div>
                    </div>
                    <button type="submit" className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold">Save Profile</button>
                </form>
            </div>
        </div>
    );
  ...
  if (viewingProfile) {
    return (
      <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        <button onClick={() => setViewingProfile(null)} className="mb-6 flex items-center gap-2 text-slate-600 hover:text-emerald-600 font-medium"><ArrowLeft className="w-5 h-5" /> Back to Teachers</button>
          ...
          <div className="bg-slate-900 text-white p-10 relative overflow-hidden">
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10">
              ...
              <div className="text-center md:text-left flex-1">
                <h1 className="text-4xl font-black mb-2">{viewingProfile.name}</h1>
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-slate-300">
                  <div className="flex items-center gap-1 bg-white/10 px-3 py-1.5 rounded-full"><Star className="w-4 h-4 text-amber-400 fill-amber-400" /><span className="font-bold">{viewingProfile.rating}</span></div>
                  <div className="flex items-center gap-1 bg-white/10 px-3 py-1.5 rounded-full"><Award className="w-4 h-4" /><span>{viewingProfile.experience} Exp.</span></div>
                  {viewingProfile.isVetted && <div className="flex items-center gap-1 bg-emerald-600/20 text-emerald-400 px-3 py-1.5 rounded-full border border-emerald-600/30"><ShieldCheck className="w-4 h-4" /><span>Vetted Tutor</span></div>}
                </div>
              </div>
              <div className="flex flex-col gap-3 min-w-[160px]">
                <div className="text-center mb-2"><span className="text-3xl font-black text-white">{viewingProfile.price}</span><span className="text-slate-400 text-sm">/hr</span></div>
          ...
          <div className="p-10 grid md:grid-cols-3 gap-12">
            <div className="md:col-span-2 space-y-10">
              ...
              {viewingProfile.videoUrl && (
                <section>
                  <h3 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-3"><span className="w-1.5 h-8 bg-emerald-600 rounded-full"></span> Watch Introduction</h3>
      ...
      <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
             <div className="max-w-xl">
                <h2 className="text-3xl font-black text-slate-900 mb-2 tracking-tight">Expert Quran Teachers</h2>
                <p className="text-slate-500">Discover and book verified teachers from around the world.</p>
             </div>
          ...
          <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-slate-50">
             {['All', ...CATEGORIES].map(cat => (
               <button 
                  ...
                  className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${activeCategory === cat ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
               >{cat}</button>
             ))}
          </div>
      </div>
          ...
          <div key={teacher.id} onClick={() => setViewingProfile(teacher)} className="bg-white rounded-3xl shadow-md overflow-hidden hover:shadow-2xl transition-all border border-slate-100 group cursor-pointer flex flex-col h-full">
            ...
            <div className="p-6 flex-1 flex flex-col">
              <div className="flex justify-between items-start mb-1">
                ...
                <span className="text-emerald-600 font-black">{teacher.price}</span>
              </div>
              <p className="text-slate-500 font-medium mb-4 text-sm">{teacher.specialty}</p>
              <div className="space-y-2 mb-6 text-sm">
                <div className="flex items-center gap-2 text-slate-600"><Award className="w-4 h-4 text-emerald-500" /><span>{teacher.experience} Experience</span></div>
              ...
              <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-50">
                 <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{teacher.category}</span>
                 <button onClick={(e) => handleBookClick(teacher, e)} className="bg-slate-900 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-xl text-sm font-bold transition-colors flex items-center gap-2 group-hover:bg-emerald-600">Book Now <ChevronRight className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white rounded-3xl shadow-2xl max-w-lg w-full p-8 animate-in fade-in zoom-in duration-300">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-black text-slate-900">Book Session</h3>
              <button onClick={() => setShowBookingForm(false)} className="text-slate-400 hover:text-slate-600 transition-colors"><X className="w-7 h-7" /></button>
            </div>
            
            <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8 flex items-center gap-4">
                <img src={selectedTeacher.image} className="w-16 h-16 rounded-xl object-cover" alt="" />
                <div>
                   <p className="font-black text-slate-900">{selectedTeacher.name}</p>
                   <p className="text-sm text-slate-500">Booking: {selectedTeacher.price}/session</p>
                </div>
            </div>
            ...
            <form onSubmit={handleConfirmBooking} className="space-y-5">
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="date" required className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none" />
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="time" required className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none" />
                </div>
              </div>
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-1">Duration</label>
                <select className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none">
                  <option value="30">30 minutes</option>
                  <option value="60">60 minutes</option>
                </select>
              </div>
              
              <button type="submit" className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition-all flex items-center justify-center gap-2">
                Confirm Booking
              </button>
            </form>
          </div>
